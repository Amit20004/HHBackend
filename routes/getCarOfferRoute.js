...
// =================== GET All ===================
router.get('/car-offers', asyncHandler(async (req, res) => {
...
// =================== PUT ===================
// Make similar changes to your PUT endpoint


// =================== PUT ===================
// Update car offer with optional image upload
router.put(
  '/car-offers/:id',
  upload.fields([
    { name: 'card_image', maxCount: 1 },
    ...
    const [result] = await db.query(
      `UPDATE car_offers 
       SET card_image = COALESCE(?, card_image),
           ...
           thumbnail_heading = ?, thumbnail_content = ?, 
           price = ?, car_name = ?, features = ?, updated_at = NOW()
       WHERE id = ?`,
      [
        cardImage,
        thumbnailImage,
        ...
        price,
        car_name,
        JSON.stringify(features ? JSON.parse(features) : []),
        id
      ]
    );

    if (result.affectedRows === 0) {
      return res.status(404).json({ success: false, message: 'Car offer not found' });
    }

    res.status(200).json({ success: true, message: 'Car offer updated' });
  })
);


// =================== DELETE ===================
router.delete('/car-offers/:id', asyncHandler(async (req, res) => {
  const { id } = req.params;

  const [result] = await db.query(`DELETE FROM car_offers WHERE id = ?`, [id]);

  if (result.affectedRows === 0) {
    return res.status(404).json({ success: false, message: 'Car offer not found' });
  }

  res.status(200).json({ success: true, message: 'Car offer deleted' });
}));
